                ...
                if bt_req == "FULL_SCAN":
                    ...
                    asyncio.create_task(backtester.run_full_scan(resume=False))
                     ...
                     if Config.DEEPSEEK_API_KEY:
                        ...
                        asyncio.create_task(run_research_session(client, backtester, researcher, resume=False))
                     else:
                        logger.error("No API Key for AI Research")
                        state.update_scan_progress(100, 100, "Error: No API Key", status="complete")

                elif bt_req == "AI_RESEARCH_RESUME":
                     # NEW: Handle Resume
                     if Config.DEEPSEEK_API_KEY:
                        researcher = AIResearcher(api_key=Config.DEEPSEEK_API_KEY)
                        asyncio.create_task(run_research_session(client, backtester, researcher, resume=True))
                     else:
                        logger.error("No API Key for AI Research")
                        state.update_scan_progress(100, 100, "Error: No API Key", status="complete")
